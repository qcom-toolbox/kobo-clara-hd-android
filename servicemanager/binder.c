                 ...
                 uint32_t *ptr, uint32_t size, binder_handler func)
{
    ...
    uint32_t *dbg_start = ptr;
    uint32_t dbg_size = size;
    ...
    while (ptr < end) {
        ...
        case BR_DEAD_REPLY:
            ...
            break;
        default: {
            unsigned wordidx = (unsigned)(ptr - 1 - dbg_start);
            if (wordidx == 0) {
                /* Reproducible kernel/toolchain quirk on this build: the
                 * first word of a fresh read is corrupted (observed to
                 * equal the read buffer's own address) instead of the
                 * BR_NOOP the driver source says it writes there when
                 * consumed==0. Word 1 onward is consistently valid. Treat
                 * word 0 as the no-op it's supposed to be and keep going,
                 * rather than aborting a transaction we can otherwise
                 * parse correctly. */
                ALOGE("parse: word 0 corrupt (0x%08x), treating as BR_NOOP workaround\n", cmd);
                break;
            }
            ALOGE("parse: OOPS %u at word %u of %u (size=%u bytes)\n",
                  cmd, wordidx, dbg_size / 4, dbg_size);
            {
                uint32_t i;
                for (i = 0; i < dbg_size / 4; i++)
                    ALOGE("  word[%u] = 0x%08x\n", i, dbg_start[i]);
            }
            return -1;
        }
